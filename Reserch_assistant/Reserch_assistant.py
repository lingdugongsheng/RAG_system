...
import json                        # 处理 JSON 数据，模拟搜索结果和报告结构
...
import time                        # 时间模块，用于重试延迟

import dotenv                      # 从 .env 文件加载环境变量（API 密钥等）
from langchain_openai import ChatOpenAI  # OpenAI 兼容的聊天模型接口，此处对接 DeepSeek
...
def safe_parse_json(text: str, default: dict = None) -> dict:
    """
    安全解析 LLM 返回的文本，提取 JSON 对象。
    支持直接 JSON、Markdown 代码块、以及最外层花括号内容。
    解析失败时返回默认值，避免程序崩溃。
    """
    if default is None:
        default = {}                     # 默认返回空字典
    ...
    try:
        ...
        pass                             # 直接解析失败则继续尝试其他策略

    # 提取 Markdown JSON 代码块（```json ... ```）
    json_match = re.search(r'```(?:json)?\s*([\s\S]*?)\s*```', content)
    if json_match:
        content = json_match.group(1).strip()  # 获取代码块内部的内容
    else:
        # 贪婪匹配最外层花括号（DOTALL 模式使 . 匹配换行符）
        brace_match = re.search(r'\{.*\}', content, re.DOTALL)
        if brace_match:
            content = brace_match.group(0)    # 获取花括号包裹的内容

    try:
        return json.loads(content)           # 再次尝试解析
...
def get_model():
    """获取 ChatOpenAI 模型的单例实例，避免重复创建"""
    global _MODEL_CACHE                  # 声明要修改全局变量
    if _MODEL_CACHE is not None:
        return _MODEL_CACHE              # 缓存存在则直接返回
    _MODEL_CACHE = ChatOpenAI(
        model="deepseek-chat",           # 使用的模型名称
        temperature=0.3,                 # 较低温度使生成更稳定、更严谨
        max_tokens=2000,                 # 增加最大 token，以支持生成完整章节
...
def llm_call_with_retry(prompt_messages, max_retries=3, delay=1.5):
    """
    带重试的 LLM 调用，防止临时网络故障导致任务失败。
    尝试 max_retries 次，每次失败后等待 delay 秒。
    """
    for attempt in range(1, max_retries + 1):
        try:
            response = get_model().invoke(prompt_messages)   # 调用模型
            return response
        except Exception as e:
            logger.error(f"LLM 调用失败 (尝试 {attempt}/{max_retries}): {e}")
            if attempt == max_retries:                       # 最后一次尝试也失败则抛出异常
                raise
...
def create_research_assistant():
    ...
    def should_continue(state: ResearchState) -> Literal["continue", "complete"]:
        """如果质量评分 >= 7.5 或已迭代 2 次，则结束；否则返回信息收集重新搜索更高质量资料"""
        score = state.get("quality_score", 0)
        iterations = state.get("iteration_count", 0)
        if score >= 7.5 or iterations >= 2:

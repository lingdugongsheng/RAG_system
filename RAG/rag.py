...
import dotenv                      # 从 .env 文件加载环境变量（API密钥等）
...
def get_embeddings():
    ...
    try:
        if DEEPSEEK_API_KEY:   # 如果有DeepSeek API密钥，使用其嵌入模型
            from langchain_openai import OpenAIEmbeddings
            _EMBEDDINGS_CACHE = OpenAIEmbeddings(
                model="deepseek-embedding-v1",  # DeepSeek提供的嵌入模型
...
def list_persisted_collections(persist_directory: str) -> List[str]:
    """
    列出指定持久化目录下所有的 Chroma 集合名称（即子目录名）。
    用于管理多个索引。
    """
    if not persist_directory or not os.path.exists(persist_directory):
        return []   # 目录不存在或未配置则返回空列表
    try:
        # 遍历目录，筛选出子目录作为集合名
        return [
            item for item in os.listdir(persist_directory)
            if os.path.isdir(os.path.join(persist_directory, item))
        ]
    except Exception as e:
...
class Retriever:
    ...
    def retrieve_by_similarity(self, query: str, threshold: float = 0.7) -> List[Document]:
        """
        带相似度阈值的检索：先取 top_k * 2 个候选，再筛选出相似度 >= threshold 的文档。
        """
        all_results = self.vector_store.similarity_search_with_relevance_scores(
            query=query,
            k=self.config.top_k * 2
        )
        filtered_docs = []
        for document, similarity_score in all_results:
            if similarity_score >= threshold:    # 只保留达到阈值的文档
